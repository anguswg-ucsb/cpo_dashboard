...
# distr_num <- c(64,1, 4, 2, 5, 6, 7, 8, 9, 80, 23)
...
independent <- c("prcp", "prcp_pct_norm", "aet", "pet", "soilm", "tavg", "tmax",
                 "pdsi","pdsi_gridmet", "eddi1","eddi3", "eddi6", "eddi12", 
                 "spi1", "spi3", "spi6", "spi9", "spi12",  "swe_max")

independent2<- c("swe_max", "pdsi", 
                 "eddi1","eddi3", "eddi6", "eddi12", 
                 "spi1", "spi3", "spi6", "spi9", "spi12")
                 
            # "water_deficit", "spi1","spi3","spi6","spi9","spi12",  "pdsi_gridmet",  "tmax", )

# admin_year <- readRDS("C:/Users/angus/OneDrive/Desktop/lynker/CPO/data/water_rights/weighted_admin.rds")

# # short_month <- readRDS("model_data_elev_mask_v9.rds") 
...
# admin_data <- readRDS("model_data_admins.rds") %>%
#   filter(year == 2012) 
breaks <- readRDS("admin_breaks.rds")


# # # dataframe w/ admin splits
# admin_data <- readRDS("model_data_admins2.rds") %>%
#   filter(year == 2012)
...
# JOIN MODEL DATA WITH DEMAND-ADMIN DATA
...
# Initialize Maps 
output$districtMapMLR     <- renderLeaflet({ basemap(shp = shp) })
# output$districtMapLM     <- renderLeaflet({ basemap(shp = shp) })

# output$bubbleMap       <- renderLeaflet({ basemap(shp = shp) })
```
...
**Default admin number depends on the cumulative annual demand of all of the admins in the specified district**
...
Build a regresion model from a set of candidate predictor variables by entering and removing predictors based on p values, in a stepwise manner until there is no variable left to enter or remove any more.
           ...
           # } else if(outcome5() == "short_log") {
                ...
                stk_hc <- 
                  highchart() %>%
                    ...
                    hc_add_series(
                        data = ws_wide, name = "Shortage",
                        type = 'column', hcaes(x = year, y = short), yAxis = 1, fillOpacity = 0.3) %>%
                    hc_add_series(
                        data = ws_wide, name = "Augmented supply",
                        type = 'column', hcaes(x = year, y = `Supply Augmented`), yAxis = 1, fillOpacity = 0.3) %>%
                    hc_add_series(
                        data = ws_wide, name = "Direct Flow Supply",
                        type = 'column',  hcaes(x = year, y = `Supply Direct flow`), yAxis = 1,  fillOpacity = 0.3) %>%
                    hc_add_series(
                    ...
                    hc_add_series(
                      ...
                      hc_colors(c("#E18686",  "#70BCE2", "#2984B2", "black", "black", "black")) %>%
