          ...
          if(nrow(pt_intersect) == 0) {
           ...
           # } else if(outcome5() == "short_log") {
                ...
                district_data <- model_data %>%
                  ...
                  lm_step  <- mlr_vfit$model
                  
                  fitted   <- lm_step$fitted.values # fitted values
                  observed <- lm_step$model %>% dplyr::select(short_dir_norm) # Observed values
                
                 #  rsquared <- lm_step$metrics[1] %>%  round(3) %>%   as.character()
                
                 # observed vs. fitted dataframe
                  pred_df <- data.frame(
                                  fitted = fitted,
                                  observed = observed$short_dir_norm
                                  ) %>%
                             mutate(
                                  fitted    = 10^fitted,
           ...
           # } else if(outcome5() == "short_log") {
                ...
                district_data <- model_data %>%
                  ...
                  if(length(mlr_vfit$predictors) == 1) {
                      ...
                      ws_predictors <- model_data %>%
                          filter(district == district_id$district) %>%
                          # filter(district == 72)  %>%
                          ungroup() %>%
                          dplyr::select(year, names(lm_step$model)[2]) %>%
                ...
                # old <- c("swe_max", "prcp", "pdsi","eddi1", "eddi3", "eddi6", "eddi12", "spi1", "spi3", "spi6", "spi9",  "spi12", "tavg","tmax", "tmin","pet", "soilm"  ) 
                
                # new <-c(    "SWE maximum (in)", "Precipitation (mm)","PDSI", "EDDI 1 month", "EDDI 3 month", "EDDI 6 month", "EDDI 12 month", "SPI 1 month", "SPI 3 month", "SPI 6 month", "SPI 9 month","SPI 12 month","Average temperature (C)", "Maximum temperature (C)", "Minimum temperature (C)",   "Potential Evapotranspiration (mm)", "Soil moisture (mm)" )
                 ...
                 highchart() %>%
                      ...
                      hc_add_series(
                            ...
                            name = name_df$clean_name[2],
                             hcaes(x = year,  y = !!names(ws_predictors)[3]),
                            # hcaes(x = year, y = !!lm_vfit$predictors[2]),
                            type = 'line',
    ...
    click <- input$districtMapLM_click %>%
          ...
          if(nrow(pt_intersect) == 0) {
              NULL
                ...
                trans_log <- trans_log %>%
                      # dplyr::select(district, short_dir_norm, prcp)
                      dplyr::select(district, depVar(), climVar())
...
##### **2.** Water nodes for the selected district will load on the bottom map.
...
```{r}
highcharter::highchartOutput("adminShortagePlot")
bsPopover("adminShortagePlot", title = "Total annual water shortages per admin", content = "This timeseries shows an estimation of the volume of direct flow shortages each user in the model node experienced. Remember that direct flow shortages express the difference between demand and direct flow supply, or water coming directly from water rights. Many users also acquire augmentation water in order to make up for this difference. Therefore, actual shortages are less than what is visualized here.", "left", options = list(container = "body"))
tags$style(HTML("
                .tooltip > .tooltip-inner {
                width: 400px;
                color: black;
...
observeEvent(input$nodeMap_marker_click, {
    ...
    admin_short_hc <- highchart() %>% 
       hc_plotOptions(
        ...
        hc_title(text = "Shortages",  style = list(fontSize = 20, fontWeight = "bold", color = "black")) %>% 
        # hc_colors(viridis::mako(n = length(node_split_short), direction = 1)) %>%
        hc_colors(viridis::cividis(n = length(node_split_short))) %>%
        hc_xAxis(categories = node_split_short[[1]][1],
                 tickInterval = 2,
                 labels = list( style = list(fontSize = 14, fontWeight = "bold", color = "black"))) %>% 
        hc_yAxis(title = list(
                            text =  "Direct flow shortage (AF)",
                            style = list( style = list(fontSize = 14, fontWeight = "bold", color = "black"))),
                 labels = list( style = list(fontSize = 14, fontWeight = "bold", color = "black"))) %>%  
        hc_chart(plotBorderWidth = 0.5, plotBorderColor = '#b4b4b4', height = NULL)
...
observeEvent(input$nodeMap_marker_click, {
    ...
    admin_dem_hc <- highchart() %>%
      highchart() %>% 
          ...
          hc_title(text = "Demand",  style = list(fontSize = 20, fontWeight = "bold", color = "black")) %>% 
          # hc_colors(viridis::mako(n = length(node_split_demand), direction = 1)) %>%
          hc_colors(viridis::cividis(n = length(node_split_demand))) %>%
          hc_xAxis(categories = node_split_demand[[1]][1],
                   tickInterval = 2,
      ...
      click <- input$districtMapFuture_click %>%
          ...
          if(nrow(pt_intersect) == 0) {
              ...
              #          # ---- Prep data for model ----
                ...
                #        # ---- Prep data for model ----
                #   mod_df <- mod_df %>%
                #       # dplyr::select(short, prcp, tavg, tmax, pdsi, spi1, spi3, spi6, spi9, spi12)
                ...
                #   # MLR w/ VIF reduction + stepwise regression
                #   mlr_vfit <- lm(
                #      as.formula(paste0(futureDepVar()," ~ .")), data = mod_df) %>% 
                #              # short~., data = mod_df) %>% 
                #              ols_step_forward_p()
                #   lm_step <- mlr_vfit$model
                #   # calc VIF
                ...
                #   # lm_step <- lm(
                #   #       as.formula(paste(
                #   #                "short", paste(vf_df$rowname, collapse=" + "), sep=" ~ ")),
                #   #               data = mod_df) 
                #     # as.formula(paste(
                #     #              futureDepVar(), paste(vf_df$rowname, collapse=" + "), sep=" ~ ")),
                #     #             data = mod_df) 
                #   
                #   # lm_fit <- lm_run$fit[[1]]
                # 
                ...
                #         # hc_yAxis(title = list(text = "Water volume (AF)"), min = 0) %>%
                #         # hc_yAxis_multiples(
                #         #     list(title = list(
                #         #       text = name_df$clean_name[1],
                #         #       style = list(fontWeight = "bold",  fontSize = '1.2em')
                ...
                #             name = name_df$clean_name[1],
                #             type = 'line',
                #               hcaes(x = year,  y = !!names(predictor_ts)[2]),
                #             # hcaes(x = year,  y = !!lm_run$predictors[1]),
                #             yAxis = 0,
                #             fillOpacity = 0.1) %>%
                #         hc_add_series(
                #             data = predictor_ts,
                #             name = name_df$clean_name[2],
                #                  hcaes(x = year,  y = !!names(predictor_ts)[3]),
                #             # type = 'line', hcaes(x = year, y =  !!lm_run$predictors[2]),
                #             yAxis = 1, fillOpacity = 0.1) %>%
                #         hc_add_series(
                #             data = dplyr::arrange(impact_ts, year),
