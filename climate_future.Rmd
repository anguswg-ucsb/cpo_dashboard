...
### Text
```{r}
verbatimTextOutput("reactText")
```

```{r context = "server"}
xxchange <- reactive({
    paste(input$districtMapFuture_click ,  input$climChangeVarInput)
})

output$reactText <- renderText({
  xxchange()
})
  
predictImpactsData <- eventReactive(xxchange(), {
   # if(!is.null(input$districtMapFuture_click)) {
      click <- input$districtMapFuture_click %>%
            data.frame() %>%
            dplyr::select(lat,lng)
...
library(shiny)
library(flexdashboard)
# install.packages(c(shiny, flexdashboard, ggplot2, kableExtra, broom, sf,tools,highcharter, olsrr, formattable, shinyalert, shinyBS, reactable, tidymodels, reactablefmtr, bsplus))
# library(farver)
# Data libraries
library(tidyr)
library(dplyr)
...
# water volumes by individual admin
by_admin <- readRDS("by_admin.rds")

# impacts & indicator dataset for modeling, yearly timescale
model_data <- readRDS("model_data.rds")

# timeseries plot data
ts_data <- readRDS("timeseries_data.rds")
...
font <- list(size = 12, color = "white") # family      = "DM Sans",
          ...
          if(nrow(pt_intersect) == 0) {
           ...
           # } else if(outcome5() == "short_log") {
                ...
                #             ) %>%
                  ...
                  # mod_df <- point_data %>%
                  #       ungroup() %>%
                  #       dplyr::select(short_dir_norm, prcp, pdsi, swe_max, pet, tavg, spi1, spi3, spi6, spi9, spi12, eddi1, eddi3, eddi6, eddi12)
                ...
                district_data <- model_data %>% 
                  ...
                  # rsquared <- mlr_vfit$metrics[1] %>%  round(3) %>%   as.character()
                ...
                pred_hc <- 
                 highchart() %>%
                      hc_plotOptions(column = list(stacking = 'normal'),
                                     line = list(marker = list(enabled = FALSE), lineWidth = 7)) %>%
